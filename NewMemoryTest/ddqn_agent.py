...
class DDQN_Agent:
    def __init__(self, useDepth=False):
        ...
        self.steps_done = 0
        self.max_steps = 400
        self.PM = PrioritizedMemory(10000, (84,86), [1], device,
                 gamma=self.gamma, multi_step=3, alpha=0.6, beta=0.4,
    ...
    def append_sample(self, state, action, reward, next_state,done):
        ...
        self.PM.append(state, action, reward, next_state, done, error,
               episode_done=False)

    def learn(self):
       # if self.memory.tree.n_entries < self.batch_size:
      #      return
        if self.PM.getCount() < self.batch_size:
            return
        #states, actions, rewards, next_states, idxs, is_weights = self.memory.sample(self.batch_size)

        states, actions, rewards, next_states, idxs, is_weights = self.PM.sample(self.batch_size)
        states = tuple(states)
        ...
        for i in range(self.batch_size):
            ...
            self.PM.update_priority(idx, errors[i])
                ...
                if done:
                    print("----------------------------------------------------------------------------------------")
                     ...
                     #   print("Training will start after ", self.batch_size - self.memory.tree.n_entries, " steps.")
                        #break
                    if self.PM.getCount() < self.batch_size:
                        print("Training will start after ", self.batch_size - self.PM.getCount(), " steps.")
                        break
                    print(
