...
class DroneEnv(object):
    ...
    def step(self, action):
        ...
        result, done = self.compute_reward(quad_state, quad_vel, collision, 0,self.dest, 0, state[3:])
    ...
    def get_obs(self, quad_state):
        ...
        if self.useLidar:
            ...
            obs = np.concatenate((obs, np.zeros((10,3))), axis=0)
        return obs
    ...
    def compute_reward(self, quad_state, quad_vel, collision, obstacles, goal, power_usage, lidar):
        ...
        if collision:
            ...
            # Dynamic scaling of rewards and penalties based on proximity to goal
                ...
                self.last_dist = dist
            print(f"Exploration Threshold : {self.threshold}")
        ...
        print(f"Lowest Lidar Distance : {self.get_lidar_dist(lidar[0])}")
        if (lidar[0] != [0,0,0]).all() and self.get_lidar_dist(lidar[0]) < 7:
            reward -= 7/self.get_lidar_dist(lidar[0])
