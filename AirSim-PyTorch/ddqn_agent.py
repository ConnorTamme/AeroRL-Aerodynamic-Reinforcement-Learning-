...
class DQN(nn.Module):
    ...
    def forward(self, x):
        #puts x through each layer
        x = F.relu(self.conv1(x))#F.relu applies a rectified linear function on each element
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        x = x.view(x.size(0), -1) #x.view gives a tensor with the same elements but a different shape
                                  #-1 means to infer the size from other dims. x.size(0) is the
                                  #size of x's most significant dimension. This is removing all
                                  #dims past the second one (ex) [3,4,5] becomes [3,20])
        x = F.relu(self.fc4(x))
        return self.fc5(x)
